"""
Script de predicción usando el modelo entrenado (migrado desde IA-module).
...
from src.infrastructure.ml._utils import logger
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            print("\nUsage:")
            print("  python scripts/ml/predict.py '<incident_description>'")
            print("\nEjemplos:")
            print("  python scripts/ml/predict.py 'Category: Hardware Impact: Critical'")
        ...
        logger.info("\nOpciones:")
        logger.info("  python scripts/ml/train.py              # Entrena con MiniLM")
        return 1
    except Exception as e:
        logger.error(f"Error: {str(e)}", exc_info=True)
        return 1
